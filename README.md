...
## Overview
This project documents the deployment of a secure Ubuntu-based web server on AWS EC2 using Nginx, UFW, OpenSSH hardening, and Let's Encrypt.

The deployment serves:
- a static HTML page on `/`
- a JSON API on `/api`

It also applies basic host-level hardening and exposes only the minimum required network ports.
